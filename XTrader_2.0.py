...
def import_googlesheet():
    try:
        ...
        # 번호, 종목명, 매수모니터링, 비중, 시가위치, 매수가1, 매수가2, 매수가3, 기본매도전략, 목표가
        ...
        for row in row_data[1:]:
            try:
                ...
                if row[idx_buyprice] == '': raise Exception('매수가1 오류')
            except Exception as e:
                name = ''
                code = ''
                market = ''
                if str(e) != '매도전략 오류' and str(e) != '매수가1 오류': e = '종목명 오류'
                print('구글 매수모니터링 시트 종목명 오류 : %s, %s' % (row[1], e))
                logger.error('구글 매수모니터링 시트 오류 : %s, %s' % (row[1], e))
                Telegram('[XTrader]구글 매수모니터링 시트 오류 : %s, %s' % (row[1], e))
...
class CTradeShortTerm(CTrade):  # 로봇 추가 시 __init__ : 복사, Setting, 초기조건:전략에 맞게, 데이터처리~Run:복사
    ...
    def buy_strategy(self, code, price):
        ...
        if condition <= 3: # 매수조건 4,5,6은 미사용(20.11.12)
            ...
            if self.주문번호_주문_매핑.get(주문번호) is not None:
                ...
                if 미체결수량 == 0:
                    try:
                        ...
                        # 분할매수라도 한번 매수가 되고 나면 매도할종목 변수에 추가
                        elif self.Stocklist[종목코드]['매수주문완료'] < self.Stocklist[종목코드]['매수가전략'] and 종목코드 not in self.매도할종목:
                            self.매도할종목.append(종목코드)
