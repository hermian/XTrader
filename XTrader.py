...
class CPortStock(object):
    def __init__(self, 매수일, 종목코드, 종목명, 매수가, 보유일, 매도구간별조건, 매도구간=1, 매도가=0, 수량=0):
        self.매수일 = 매수일
        self.종목코드 = 종목코드
        self.종목명 = 종목명
        self.매수가 = 매수가
...
class CTrade(object):
    ...
    def KiwoomSendOrder(self, sRQName, sScreenNo, sAccNo, nOrderType, sCode, nQty, nPrice, sHogaGb, sOrgOrderNo):
        ...
        try:
            ...
            logger.error('CTradeShortTerm_KiwoomSendOrder : %s' % e)
        # -거래구분값 확인(2자리)
        #
        # 00 : 지정가
        # 03 : 시장가
    ...
    def OnReceiveTrData(self, sScrNo, sRQName, sTRCode, sRecordName, sPreNext, nDataLength, sErrorCode, sMessage, sSPlmMsg):
        ...
        try:
            ...
            logger.error('CTradeShortTerm_OnReceiveTrData : %s' % e)
    ...
    def OnReceiveChejanData(self, sGubun, nItemCnt, sFidList):
        ...
        try:
            ...
            logger.error('CTradeShortTerm_OnReceiveChejanData : %s' % e)
    ...
    def OnReceiveRealData(self, sRealKey, sRealType, sRealData):
        ...
        try:
            ...
            logger.error('CTradeShortTerm_OnReceiveRealData : %s' % e)
    ...
    def 정액매수(self, sRQName, 종목코드, 매수가, 매수금액):
        ...
        try:
            ...
            logger.error('CTradeShortTerm_정액매수 : %s' % e)
    ...
    def 정량매도(self, sRQName, 종목코드, 매도가, 수량):
        ...
        try:
            ...
            logger.error('CTradeShortTerm_정량매도 : %s' % e)
...
class CTradeShortTerm(CTrade):  # 로봇 추가 시 __init__ : 복사, Setting, 초기조건:전략에 맞게, 데이터처리~Run:복사
    ...
    def save_history(self, code, status):
        ...
        try:
            code_row = history_sheet.findall(self.portfolio[code].종목명)[-1].row # 종목명이 있는 모든 셀을 찾아서 맨 아래에 있는 셀을 선택
    ...
    def set_portfolio(self, code, condition):
        try:
            ...
            logger.error('CTradeShortTerm_set_portfolio : %s' % e)
    ...
    def 실시간데이타처리(self, param):
        try:
            ...
            logger.error('CTradeShortTerm_실시간데이타처리 : %s' % e)
    ...
    def Run(self, flag=True, sAccount=None):
        ...
        #         del self.portfolio[code].매수비용
        #         del self.portfolio[code].매도비용
...
class CTradeCondition(CTrade): # 로봇 추가 시 __init__ : 복사, Setting / 초기조건:전략에 맞게, 데이터처리 / Run:복사
    ...
    def sell_strategy(self, code, price):
        ...
        # 1. 보유일 제한 매도 전략 : 보유기간이 보유일 이상일 경우 전량 매도 실행
        보유기간 = periodcal(self.portfolio[code].매수일)
        if 보유기간 >= int(self.portfolio[code].보유일):
            result = True
            sell_price = 전일종가
    ...
    def Setting(self, sScreenNo, 포트폴리오수, 조건식인덱스, 조건식명, 단위투자금, 매수방법, 매도방법, 종목리스트):
            ...
            if 종목코드 in self.매도할종목:
                if self.portfolio.get(종목코드) is not None and self.주문실행중_Lock.get('S_%s' % 종목코드) is None:
                    ...
                    if sell_check == True:
                        ...
                        if result == True:
                            self.주문실행중_Lock['S_%s' % 종목코드] = True
                            Telegram('CTradeCondition정액매도 : sRQName=%s, 종목코드=%s, 매도가=%s, 수량=%s' % (
                                'S_%s' % 종목코드, 종목코드, 현재가, self.portfolio[종목코드].수량))
                            logger.info('CTradeCondition정액매도 : sRQName=%s, 종목코드=%s, 매도가=%s, 수량=%s' % (
                                'S_%s' % 종목코드, 종목코드, 현재가, self.portfolio[종목코드].수량))
                        else:
                            Telegram('CTradeCondition 정액매도실패 : sRQName=%s, 종목코드=%s, 매도가=%s, 수량=%s' % (
                                'S_%s' % 종목코드, 종목코드, 현재가, self.portfolio[종목코드].수량))
                            logger.info('CTradeCondition 정액매도실패 : sRQName=%s, 종목코드=%s, 매도가=%s, 수량=%s' % (
                                'S_%s' % 종목코드, 종목코드, 현재가, self.portfolio[종목코드].수량))
            ...
            if current_time < '10:00:00':
                if 종목코드 in self.매수할종목 and 종목코드 not in self.금일매도종목:
                    if len(self.portfolio) < self.최대포트수 and self.portfolio.get(종목코드) is None and self.주문실행중_Lock.get('B_%s' % 종목코드) is None:
                        if 현재가 <= 전일종가 * (1-(abs(self.매수가비율)/100)) and (self.단위투자금 // 현재가 > 0):
                            (result, order) = self.정액매수(sRQName='B_%s' % 종목코드, 종목코드=종목코드, 매수가=현재가, 매수금액=self.단위투자금)
                            if result == True:
                                self.portfolio[종목코드] = CPortStock(종목코드=종목코드, 종목명=종목명, 매수가=현재가,
                                                                  매도구간별조건=self.Stocklist['전략']['매도구간별조건'], 보유일=self.Stocklist['전략']['보유일'],
                                                                  매수일=datetime.datetime.now().strftime('%Y/%m/%d %H:%M:%S'))
                                self.주문실행중_Lock['B_%s' % 종목코드] = True
                                logger.info(
                                    '정액매수 : sRQName=%s, 종목코드=%s, 매수가=%s, 단위투자금=%s' % ('B_%s' % 종목코드, 종목코드, 현재가, self.단위투자금))
                            else:
                                logger.info('정액매수실패 : sRQName=%s, 종목코드=%s, 매수가=%s, 단위투자금=%s' % (
            ...
            if self.주문번호_주문_매핑.get(주문번호) is not None:
                ...
                if 미체결수량 == 0:
                    try:
                        ...
                        logger.info('CTradeCondition %s 매수 완료 : 매수/주문%s Pop, 매도 Append  ' % (종목코드, 주문))
                    except Exception as e:
                        Telegram('[XTrader]CTradeCondition 체결처리_매수 POP에러 목명:%s ' % P.종목명)
                        logger.error('CTradeCondition 체결처리_매수 POP에러 종목명:%s ' % P.종목명)
        ...
        if param['매도수구분'] == '1':
            if self.주문번호_주문_매핑.get(주문번호) is not None:
                ...
                try:
                    ...
                    logger.error('CTradeCondition 체결처리_매도 매매이력 Error : %s' % e)
                ...
                try:
                    ...
                    logger.error('CTradeCondition 체결처리_매도 POP에러 종목명:%s' % param['종목명'])

        # 메인 화면에 반영
        self.parent.RobotView()

    def 잔고처리(self, param):
        # print('CTradeCondition : 잔고처리')
    ...
    def Run(self, flag=True, sAccount=None):
        ...
        if flag == True:
            ...
            # if len(self.금일매도종목) > 0:
            #     Telegram("[XTrader]CTradeCondition 금일 매도 종목 손익 Upload : %s" % (self.금일매도종목))
            #     logger.info("CTradeCondition 금일 매도 종목 손익 Upload : %s" % (self.금일매도종목))
            #     self.parent.statusbar.showMessage("금일 매도 종목 손익 Upload")
            #     self.parent.DailyProfit(self.금일매도종목)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def MENU_Action(self, qaction):
        ...
        try:
            ...
            if _action == "actionExit":
                ...
                self.close()
            elif _action == "actionLogin": self.KiwoomLogin()
            elif _action == "actionLogout": self.KiwoomLogout()
                ...
                self.RobotLoad()
                self.RobotView()
            elif _action == "actionRobotSave": self.RobotSave()
    ...
    def RobotOneMonitoringStop(self):
        ...
        if robot_found.running == True:
            ...
            Slack('[XTrade]Robot_%s : 매수 모니터링 정지' % robot_found.sName)
    ...
    def RobotAdd_TradeCondition(self):
        ...
        if R.exec_():
            이름 = R.lineEdit_name.text()
            스크린번호 = int(R.lineEdit_screen_number.text())
            단위투자금 = int(R.lineEdit_unit.text())
            매수방법 = R.comboBox_buy_sHogaGb.currentText().strip()[0:2]
            매도방법 = R.comboBox_sell_sHogaGb.currentText().strip()[0:2]
            포트폴리오수 = int(R.lineEdit_portsize.text().strip())
            ...
            r.Setting(sScreenNo=스크린번호, 단위투자금=단위투자금, 포트폴리오수=포트폴리오수, 조건식인덱스 = 조건식인덱스, 조건식명 = 조건식명, 매수방법=매수방법,매도방법=매도방법, 종목리스트=종목리스트)
    ...
    def RobotEdit_TradeCondition(self, robot):
        ...
        if R.exec_():
            ...
            robot.sName = 이름
            robot.Setting(sScreenNo=스크린번호, 단위투자금=단위투자금, 포트폴리오수=포트폴리오수, 조건식인덱스=조건식인덱스, 조건식명=조건식명, 매수방법=매수방법, 매도방법=매도방법, 종목리스트=종목리스트)
